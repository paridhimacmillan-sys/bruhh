...
const formSchema = z.object({
  ...
  if (values.applicantType === "farmer") {
    ...
    if (!values.acres || values.acres <= 0) context.addIssue({ code: "custom", path: ["acres"], message: "Enter cultivated acres" });
  }
    ...
    return (
      <div className="rounded-lg border border-border bg-card p-8 text-center sm:p-12">
        ...
        <p className="mt-3 text-base text-muted-foreground">
          {text(
            "The StubbleX team will verify your details and contact you. Submitting this form does not activate an account automatically.",
            "StubbleX ਟੀਮ ਤੁਹਾਡੀ ਜਾਣਕਾਰੀ ਦੀ ਜਾਂਚ ਕਰਕੇ ਸੰਪਰਕ ਕਰੇਗੀ। ਇਹ ਫਾਰਮ ਭਰਨ ਨਾਲ ਖਾਤਾ ਆਪਣੇ ਆਪ ਚਾਲੂ ਨਹੀਂ ਹੁੰਦਾ।",
            "StubbleX टीम आपकी जानकारी की जाँच करके संपर्क करेगी। यह फ़ॉर्म भरने से खाता अपने आप सक्रिय नहीं होता।",
          )}
        </p>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            ...
            {applicantType === "farmer" && <>
              ...
              <TextField form={form} name="acres" label={text("Cultivated acres", "ਖੇਤੀ ਵਾਲੇ ਏਕੜ", "खेती के एकड़")} placeholder="12" type="number" />
              <TextField form={form} name="expectedTonnes" label={text("Expected tonnes (optional)", "ਅਨੁਮਾਨਿਤ ਟਨ (ਚੋਣਵਾਂ)", "अनुमानित टन (वैकल्पिक)")} placeholder="18" type="number" />
            </>}
          ...
          <label className="block rounded-md border border-dashed border-border p-4 text-sm">
            ...
            <span className="mt-1 block text-xs text-muted-foreground">Up to 3 PDF, JPG or PNG files · maximum 2 MB each</span>
