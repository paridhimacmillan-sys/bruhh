...
export function AdminControlCentre() {
  ...
  const adminActions = [
    ...
    ...(pendingQuantities.length ? [{ id: "quantities", title: `${pendingQuantities.length} quantity adjustments are waiting`, detail: "The operator has verified a volume change; admin approval updates the farmer listing.", meta: "Farmer updates", urgency: "today" as const, href: "/dispatch", actionLabel: "Review requests" }] : []),
  ...
  return <div className="mt-6">
    ...
    <ActionQueue items={adminActions} title="Decisions waiting for StubbleX" />
    ...
    <Tabs id="admin-control-tabs" defaultValue="overview" className="mt-6 rounded-lg border border-border bg-card p-2 sm:p-3">
      ...
      <TabsContent value="approvals" className="space-y-4 p-1 pt-3 sm:p-2 sm:pt-4">
        <Panel title="Registration requests" note="Final decisions remain with the StubbleX organization." action={<Button asChild size="sm"><Link href="/dispatch">Open full onboarding</Link></Button>}>
        ...
        <Panel title="Verified quantity adjustments" note="Operators verify the field; admins approve the new listing total." action={<Button asChild size="sm" variant="outline"><Link href="/dispatch">Review requests</Link></Button>}>
          {pendingQuantities.length ? <TableWrap><Head labels={["Farmer", "Verified by", "Previous", "Change", "New total", "Reason"]} /><tbody>{pendingQuantities.map((item) => <tr key={item.id} className="border-t"><td className="px-4 py-3 font-medium">{item.farmerName}</td><td className="px-4 py-3">{item.requestedByName}</td><td className="px-4 py-3">{number(item.previousTonnes)} t</td><td className={`px-4 py-3 ${item.additionalTonnes >= 0 ? "text-primary" : "text-destructive"}`}>{item.additionalTonnes > 0 ? "+" : ""}{number(item.additionalTonnes)} t</td><td className="px-4 py-3 font-medium">{number(item.requestedTotalTonnes)} t</td><td className="max-w-[260px] px-4 py-3 text-muted-foreground">{item.reason}</td></tr>)}</tbody></TableWrap> : <Empty>No verified quantity changes are awaiting approval.</Empty>}
        </Panel>
      </TabsContent>
