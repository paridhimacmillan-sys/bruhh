          ...
          {batches.map((batch) => (
            <tr key={batch.id} className="border-b border-border/70 last:border-0 hover:bg-secondary/45">
              <td className="px-5 py-4">
                <span className="font-mono text-xs font-medium text-foreground">Batch #{batch.id}</span>
...
export function DispatchPage() {
  const [activeTab, setActiveTab] = useState<"batches" | "orders" | "farmer_updates" | "onboarding">("batches");
  ...
  useEffect(() => {
    ...
    if (user?.role === "inspector") navigate("/inspector", { replace: true });
    if (user?.role === "operator") navigate("/operator", { replace: true });
    if (user?.role === "aggregator") navigate("/aggregator", { replace: true });
  ...
  const canAccessFarmerUpdates = canManageBatches || user.role === "operator";
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto w-full max-w-7xl px-5 py-8 md:px-8 md:py-12">
        ...
        <section className="mt-6 overflow-hidden rounded-lg border border-border bg-card" aria-labelledby="batch-ledger-title">
          <div className="flex items-center justify-between border-b border-border px-5 py-4">
            <div>
              ...
              <div className="mt-2 flex gap-2">
                ...
                {canAccessFarmerUpdates && <button onClick={() => setActiveTab("farmer_updates")} className={`rounded-full px-3 py-1.5 text-sm ${activeTab === "farmer_updates" ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground"}`}>Farmer updates</button>}
          ...
          {activeTab === "farmer_updates" && canAccessFarmerUpdates && <FarmerQuantityPanel canDecide={canManageBatches} currentUserId={user.id} />}
