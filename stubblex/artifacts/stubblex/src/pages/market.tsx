...
export function MarketPage() {
  ...
  const { data: rawLots = [], isLoading } = useListLots({ query: {
    queryKey: getListLotsQueryKey(),
    retry: false,
    refetchInterval: 15_000,
    refetchOnWindowFocus: true,
  } });
  const liveInventory = rawLots.filter((lot) => (lot.status === "available" || lot.status === "requested") && lot.tonnes > 0);
  const availableTonnes = liveInventory.reduce((sum, lot) => sum + lot.tonnes, 0);
  const yardCount = new Set(rawLots.map((lot) => lot.yardName)).size;
  const liveLots = liveInventory.length;
  ...
  return <div className="min-h-screen bg-background"><MarketHeader /><main>
    <section className="border-b border-border bg-primary text-primary-foreground"><div className="mx-auto max-w-7xl px-5 py-12 md:px-8 md:py-16"><p className="text-[0.65rem] uppercase tracking-[0.18em] opacity-70">{text("StubbleX live market", "StubbleX ਲਾਈਵ ਮੰਡੀ", "StubbleX लाइव बाज़ार")}</p><div className="mt-4 grid gap-8 lg:grid-cols-[1.3fr_0.7fr] lg:items-end"><div><h1 className="max-w-3xl font-display text-4xl tracking-tight sm:text-6xl">{text("Verified residue, ready to move.", "ਤਸਦੀਕਸ਼ੁਦਾ ਪਰਾਲੀ, ਚੁੱਕਣ ਲਈ ਤਿਆਰ।", "सत्यापित पराली, उठाने के लिए तैयार।")}</h1><p className="mt-4 max-w-2xl text-sm leading-relaxed opacity-80">{text("Browse paddy-residue lots with clear volume, pricing and pickup terms.", "ਸਾਫ਼ ਮਾਤਰਾ, ਕੀਮਤ ਅਤੇ ਚੁੱਕਾਈ ਦੀਆਂ ਸ਼ਰਤਾਂ ਵਾਲੇ ਝੋਨੇ ਦੀ ਪਰਾਲੀ ਦੇ ਲਾਟ ਵੇਖੋ।", "स्पष्ट मात्रा, मूल्य और उठान शर्तों वाले धान-पराली लॉट देखें।")}</p></div><div className="grid grid-cols-3 gap-px overflow-hidden rounded-xl border border-primary-foreground/20 bg-primary-foreground/20"><div className="bg-primary p-4"><p className="text-[0.65rem] opacity-70">{text("Available tonnes", "ਉਪਲਬਧ ਟਨ", "उपलब्ध टन")}</p><p className="mt-2 font-display text-2xl">{availableTonnes.toFixed(0)}</p></div><div className="bg-primary p-4"><p className="text-[0.65rem] opacity-70">{text("Live lots", "ਲਾਈਵ ਲਾਟ", "लाइव लॉट")}</p><p className="mt-2 font-display text-2xl">{liveLots}</p></div><div className="bg-primary p-4"><p className="text-[0.65rem] opacity-70">{text("Yards", "ਯਾਰਡ", "यार्ड")}</p><p className="mt-2 font-display text-2xl">{yardCount}</p></div></div></div></div></section>
