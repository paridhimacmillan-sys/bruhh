...
import { AlertTriangle, ArrowLeft, ArrowUpRight, CheckCircle2, MapPin, PackageCheck, ShieldCheck, Warehouse } from "lucide-react";
    ...
    <article className="flex h-full flex-col overflow-hidden rounded-lg border border-border bg-card">
      ...
      <div className="flex flex-1 flex-col p-5 sm:p-6">
        ...
        <div className="mt-auto flex items-center justify-between gap-3 pt-6">
          <Link href={`/market/${lot.id}`} className="inline-flex items-center gap-1 text-xs font-medium text-primary">{text("View passport trail", "ਪਾਸਪੋਰਟ ਰਿਕਾਰਡ ਵੇਖੋ", "पासपोर्ट रिकॉर्ड देखें")} <ArrowUpRight className="h-3.5 w-3.5" /></Link>
...
function MarketHeader() {
  const { text } = useLanguage();
  return <header className="border-b border-border/70 bg-background"><div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 md:px-8"><Link href="/" className="font-display text-lg">Stubble<span className="text-primary">X</span></Link><div className="flex items-center gap-3 sm:gap-4"><Link href="/" className="hidden text-sm text-muted-foreground sm:inline">{text("About", "ਸਾਡੇ ਬਾਰੇ", "हमारे बारे में")}</Link><Link href="/market" className="text-sm font-medium text-primary">{text("Market", "ਮੰਡੀ", "बाज़ार")}</Link><LanguageToggle /></div></div></header>;
}
...
export function MarketPage() {
  ...
  const { data: lots = [], isLoading } = useListLots({ query: { queryKey: getListLotsQueryKey(), retry: false } });
  const availableTonnes = lots.filter((lot) => lot.status === "available" || lot.status === "requested").reduce((sum, lot) => sum + lot.tonnes, 0);
  return <div className="min-h-screen bg-background"><MarketHeader /><main>
    <section className="border-b border-border bg-primary text-primary-foreground"><div className="mx-auto max-w-7xl px-5 py-12 md:px-8 md:py-16"><p className="text-[0.65rem] uppercase tracking-[0.18em] opacity-70">{text("StubbleX live market", "StubbleX ਲਾਈਵ ਮੰਡੀ", "StubbleX लाइव बाज़ार")}</p><div className="mt-4 flex flex-wrap items-end justify-between gap-6"><div><h1 className="font-display text-4xl tracking-tight sm:text-6xl">{text("Verified residue, ready to move.", "ਤਸਦੀਕਸ਼ੁਦਾ ਪਰਾਲੀ, ਚੁੱਕਣ ਲਈ ਤਿਆਰ।", "सत्यापित पराली, उठाने के लिए तैयार।")}</h1><p className="mt-4 max-w-2xl text-sm leading-relaxed opacity-80">{text("Browse traceable paddy-residue lots from Sangrur yards. Every tonne is backed by field and weighbridge records.", "ਸੰਗਰੂਰ ਦੇ ਯਾਰਡਾਂ ਤੋਂ ਪਤਾ ਲਗਾਉਣ ਯੋਗ ਝੋਨੇ ਦੀ ਪਰਾਲੀ ਦੇ ਲਾਟ ਵੇਖੋ। ਹਰ ਟਨ ਨਾਲ ਖੇਤ ਅਤੇ ਵੇਅਬ੍ਰਿਜ ਰਿਕਾਰਡ ਜੁੜਿਆ ਹੈ।", "संगरूर के यार्डों से पता लगाने योग्य धान-पराली के लॉट देखें। हर टन के साथ खेत और वेब्रिज रिकॉर्ड जुड़ा है।")}</p></div><div className="rounded-lg border border-primary-foreground/20 px-5 py-4"><p className="text-xs opacity-70">{text("Available this season", "ਇਸ ਸੀਜ਼ਨ ਉਪਲਬਧ", "इस सीज़न उपलब्ध")}</p><p className="mt-1 font-display text-3xl">{availableTonnes.toFixed(1)} t</p></div></div></div></section>
...
export function LotDetailPage() {
  ...
  const firstPassport = lot?.passportIds[0] ?? "";
  const { data: batch } = useGetBatch(firstPassport, { query: { queryKey: getGetBatchQueryKey(firstPassport), enabled: Boolean(firstPassport), retry: false } });
  ...
  return <div className="min-h-screen bg-background"><MarketHeader /><main className="mx-auto max-w-5xl px-5 py-8 md:px-8 md:py-14"><Link href="/market" className="inline-flex items-center gap-2 text-sm text-muted-foreground"><ArrowLeft className="h-4 w-4" /> {text("All lots", "ਸਾਰੇ ਲਾਟ", "सभी लॉट")}</Link><div className="mt-8 grid gap-8 lg:grid-cols-[1.1fr_0.9fr]"><section><p className="font-mono text-xs text-primary">{lot.id}</p><h1 className="mt-3 font-display text-5xl">{lot.clusterName} {text("lot", "ਲਾਟ", "लॉट")}</h1><p className="mt-3 flex items-center gap-2 text-sm text-muted-foreground"><MapPin className="h-4 w-4" /> {lot.yardName}, {lot.yardDistrict}</p><div className="mt-8 grid grid-cols-2 gap-px overflow-hidden rounded-lg border border-border bg-border"><div className="bg-card p-5"><p className="text-xs text-muted-foreground">{text("Available volume", "ਉਪਲਬਧ ਮਾਤਰਾ", "उपलब्ध मात्रा")}</p><p className="mt-2 font-display text-3xl">{lot.tonnes} t</p></div><div className="bg-card p-5"><p className="text-xs text-muted-foreground">{text("Ex-yard price", "ਯਾਰਡ ਮੁੱਲ", "यार्ड मूल्य")}</p><p className="mt-2 font-display text-3xl">{formatInr(lot.priceInrPerTonne)}/t</p></div><div className="bg-card p-5"><p className="text-xs text-muted-foreground">{text("Passport records", "ਪਾਸਪੋਰਟ ਰਿਕਾਰਡ", "पासपोर्ट रिकॉर्ड")}</p><p className="mt-2 font-display text-3xl">{lot.passportIds.length}</p></div><div className="bg-card p-5"><p className="text-xs text-muted-foreground">{text("Baled", "ਗੱਠਾਂ ਬਣੀਆਂ", "गांठें बनीं")}</p><p className="mt-2 text-sm font-medium">{formatDate(lot.baledAt)}</p></div></div><div className="mt-6 rounded-lg border border-primary/20 bg-primary/10 p-5"><PackageCheck className="h-5 w-5 text-primary" /><p className="mt-3 font-medium">{text("Every tonne in this lot carries a scannable Digital Product Passport.", "ਇਸ ਲਾਟ ਦੇ ਹਰ ਟਨ ਨਾਲ ਸਕੈਨ ਕੀਤਾ ਜਾ ਸਕਣ ਵਾਲਾ ਡਿਜ਼ਿਟਲ ਪ੍ਰੋਡਕਟ ਪਾਸਪੋਰਟ ਹੈ।", "इस लॉट के हर टन के साथ स्कैन किया जा सकने वाला डिजिटल प्रोडक्ट पासपोर्ट है।")}</p><p className="mt-1 text-sm text-muted-foreground">{text(`${lot.passportIds.length} linked field-to-yard records travel with this lot.`, `${lot.passportIds.length} ਜੁੜੇ ਖੇਤ-ਤੋਂ-ਯਾਰਡ ਰਿਕਾਰਡ ਇਸ ਲਾਟ ਨਾਲ ਹਨ।`, `${lot.passportIds.length} जुड़े खेत-से-यार्ड रिकॉर्ड इस लॉट के साथ हैं।`)}</p></div></section><aside className="rounded-[1.5rem] border border-border bg-card p-3"><div className="rounded-xl bg-primary p-6 text-primary-foreground"><div className="flex items-center justify-between"><p className="text-[0.65rem] uppercase tracking-[0.16em] opacity-70">{text("Passport preview", "ਪਾਸਪੋਰਟ ਝਲਕ", "पासपोर्ट झलक")}</p><ShieldCheck className="h-5 w-5" /></div><h2 className="mt-5 font-display text-3xl">{text("Verified origin", "ਤਸਦੀਕਸ਼ੁਦਾ ਮੂਲ", "सत्यापित मूल")}</h2><p className="mt-1 break-all text-xs opacity-75">{firstPassport}</p></div><div className="p-5"><p className="eyebrow">{text("One linked batch", "ਇੱਕ ਜੁੜਿਆ ਬੈਚ", "एक जुड़ा बैच")}</p><dl className="mt-4 space-y-3 text-sm"><div className="flex justify-between"><dt className="text-muted-foreground">{text("Weight", "ਵਜ਼ਨ", "वज़न")}</dt><dd>{batch?.weightTonnes ?? "—"} t</dd></div><div className="flex justify-between"><dt className="text-muted-foreground">{text("Baled", "ਗੱਠਾਂ ਬਣੀਆਂ", "गांठें बनीं")}</dt><dd>{batch ? formatDate(batch.baledAt) : "—"}</dd></div><div className="flex justify-between"><dt className="text-muted-foreground">{text("Weighbridge", "ਵੇਅਬ੍ਰਿਜ", "वेब्रिज")}</dt><dd>{batch?.weighbridgeId ?? "—"}</dd></div></dl>{firstPassport && <Button asChild variant="outline" className="mt-6 w-full"><Link href={`/p/${firstPassport}`}>{text("Open passport trail", "ਪਾਸਪੋਰਟ ਰਿਕਾਰਡ ਖੋਲ੍ਹੋ", "पासपोर्ट रिकॉर्ड खोलें")} <ArrowUpRight className="ml-2 h-4 w-4" /></Link></Button>}</div></aside></div></main></div>;
}
