...
function initialLanguage(): Language | null {
  ...
  if (window.location.pathname.includes("/r/") || window.location.pathname.includes("/farmer/")) return "pa";
  return null;
}
...
export function PublicLanguageGate({ children }: { children: ReactNode }) {
  ...
  const { language } = useLanguage();
  const staffRoute = ["/dispatch", "/admin", "/inspector", "/operator", "/aggregator"].some((path) => location === path || location.startsWith(`${path}/`));
  if (staffRoute) return children;
