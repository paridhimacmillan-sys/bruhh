...
export async function sendAdminFarmerSms(input: {
  phone: string;
  kind: "pickup" | "payment" | "registration";
  message: string;
}): Promise<void> {
  const templateId = input.kind === "pickup"
    ? process.env.MSG91_PICKUP_TEMPLATE_ID
    : input.kind === "payment"
      ? process.env.MSG91_FARMER_TEMPLATE_ID
      : process.env.MSG91_FARMER_APPROVAL_TEMPLATE_ID;
  await sendMsg91(templateId, input.phone, { message: input.message, VAR1: input.message }, input.message);
}
