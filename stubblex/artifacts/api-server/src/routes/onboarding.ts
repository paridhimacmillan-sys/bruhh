...
router.post("/onboarding-applications/:applicationId/approve", requireAuth, async (req, res, next) => {
  ...
  try {
    const result = await db.transaction(async (tx) => {
      ...
      if (application.applicantType === "machine_partner" || application.applicantType === "logistics_operator") {
        ...
        const [partner] = existingUser
          ? await tx.update(usersTable).set({ name: application.name, role }).where(eq(usersTable.id, existingUser.id)).returning()
          : await tx.insert(usersTable).values({ name: application.name, phone: application.phone, role }).returning();
    ...
    if (approvedApplication.applicantType === "farmer") {
      ...
      console.info(`[onboarding] Partner approved for ${approvedApplication.phone}; OTP login is now enabled.`);
