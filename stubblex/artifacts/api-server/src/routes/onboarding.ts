  ...
  try {
    ...
    if (approvedApplication.applicantType === "farmer") {
      ...
      if (farmer && operator) {
        ...
        await sendFarmerEnrollmentSms({ phone: farmer.phone, name: farmer.name, reference: approvedApplication.reference, operatorName: operator.name, operatorPhone: operator.phone, listedTonnes: farmer.listedTonnes, dashboardUrl: `${baseUrl}/farmer/${farmer.accessToken}?lang=pa` });
      }
