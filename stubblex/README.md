...
StubbleX is a Punjab crop-residue marketplace coordinating farmer onboarding, field operations, payments, dispatches, and industrial buyers.
...
The protected `/admin` command centre includes farmer and partner approvals, verified quantity changes, locked pickup scheduling, payment review and reconciliation, marketplace orders, machine availability, operational alerts, cluster reports, audit history, SMS preview/resend, staff access, and pilot economics/settings. Sensitive changes are tied to the signed-in administrator.

Staff sign in with Google. Google returns a verified email and StubbleX grants access only when that exact email belongs to an active record in the `users` table. The OAuth flow uses a short-lived state cookie and PKCE. Set the Google OAuth authorised redirect URI to `${PUBLIC_BASE_URL}/api/auth/google/callback`. `BOOTSTRAP_ADMIN_EMAIL` assigns the first seeded admin to your Google email; approved machine/logistics partners receive access through the Google email submitted in their application.
...
- `artifacts/stubblex/src/components/AdminControlCentre.tsx` contains the tabbed admin command-centre interface.
- `artifacts/api-server/src/routes/admin-control.ts` contains the admin-only REST endpoints.
- `artifacts/api-server/src/lib/audit.ts` records sensitive staff, batch, order, scheduling, payment, machine and settings changes.
- `admin_settings`, `payment_reviews`, and `audit_logs` store durable operating controls; pickup locking and fleet availability live on their related records.
- Configure `MSG91_PICKUP_TEMPLATE_ID` to send pickup confirmations from the SMS centre. Without it, the complete demo message is logged safely to the server console.
...
- Keep the landing page, marketplace, receipts, and dispatch dashboard on the existing StubbleX design tokens in `index.css`.
