...
export const farmerCallbackStatusEnum = pgEnum("farmer_callback_status", [
  "pending",
  "contacted",
  "resolved",
]);

export const machineAvailabilityEnum = pgEnum("machine_availability", [
  "available",
  "assigned",
  "maintenance",
  "offline",
...
export const farmersTable = pgTable(
  "farmers",
  {
    ...
    accessToken: text("access_token").notNull().default(sql`md5(random()::text || clock_timestamp()::text)`),
  },
  (table) => [
    uniqueIndex("farmers_name_cluster_idx").on(table.name, table.clusterId),
    uniqueIndex("farmers_access_token_idx").on(table.accessToken),
  ],
);
...
export const batchesTable = pgTable("batches", {
  ...
  assignedAggregatorId: integer("assigned_aggregator_id").references(() => usersTable.id, {
    onDelete: "set null",
  }),
  ...
  pickupLockedAt: timestamp("pickup_locked_at", { withTimezone: true }),
...
export const farmerCallbackRequestsTable = pgTable("farmer_callback_requests", {
  id: serial("id").primaryKey(),
  farmerId: integer("farmer_id").notNull().references(() => farmersTable.id, { onDelete: "cascade" }),
  additionalTonnes: numeric("additional_tonnes", { precision: 9, scale: 2, mode: "number" }).notNull(),
  status: farmerCallbackStatusEnum("status").notNull().default("pending"),
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
...
export const machinesTable = pgTable("machines", {
  ...
  status: machineAvailabilityEnum("status").notNull().default("available"),
  registrationNumber: text("registration_number"),
  rateInrPerDay: integer("rate_inr_per_day"),
  lastServiceAt: timestamp("last_service_at", { withTimezone: true }),
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
});

export const paymentReviewsTable = pgTable("payment_reviews", {
  id: serial("id").primaryKey(),
  batchId: integer("batch_id").notNull().references(() => batchesTable.id, { onDelete: "cascade" }).unique(),
  status: paymentReviewStatusEnum("status").notNull().default("pending"),
  reviewedByUserId: integer("reviewed_by_user_id").references(() => usersTable.id, { onDelete: "set null" }),
  notes: text("notes"),
  reviewedAt: timestamp("reviewed_at", { withTimezone: true }),
...
export const adminSettingsTable = pgTable("admin_settings", {
  id: serial("id").primaryKey(),
  seasonName: text("season_name").notNull().default("Sangrur pilot"),
  seasonTargetTonnes: integer("season_target_tonnes").notNull().default(3400),
  farmerRateInrPerTonne: integer("farmer_rate_inr_per_tonne").notNull().default(400),
  saleRateInrPerTonne: integer("sale_rate_inr_per_tonne").notNull().default(1700),
  commissionPct: numeric("commission_pct", { precision: 5, scale: 2, mode: "number" }).notNull().default(5),
  pickupPenaltyInr: integer("pickup_penalty_inr").notNull().default(15000),
...
export const farmersRelations = relations(farmersTable, ({ one, many }) => ({
  ...
  callbackRequests: many(farmerCallbackRequestsTable),
}));
...
export const batchesRelations = relations(batchesTable, ({ one, many }) => ({
  ...
  assignedAggregator: one(usersTable, {
    relationName: "assignedAggregator",
    fields: [batchesTable.assignedAggregatorId],
    references: [usersTable.id],
  }),
  lotLinks: many(lotBatchesTable),
  paymentReviews: many(paymentReviewsTable),
}));

export const usersRelations = relations(usersTable, ({ many }) => ({
  assignedBatches: many(batchesTable, { relationName: "assignedOperator" }),
  ...
  assignedAggregatorBatches: many(batchesTable, { relationName: "assignedAggregator" }),
  paymentReviews: many(paymentReviewsTable),
  auditLogs: many(auditLogsTable),
}));

export const paymentReviewsRelations = relations(paymentReviewsTable, ({ one }) => ({
  batch: one(batchesTable, { fields: [paymentReviewsTable.batchId], references: [batchesTable.id] }),
  reviewedBy: one(usersTable, { fields: [paymentReviewsTable.reviewedByUserId], references: [usersTable.id] }),
}));

export const auditLogsRelations = relations(auditLogsTable, ({ one }) => ({
  actor: one(usersTable, { fields: [auditLogsTable.actorUserId], references: [usersTable.id] }),
}));
...
export const farmerCallbackRequestsRelations = relations(farmerCallbackRequestsTable, ({ one }) => ({
  farmer: one(farmersTable, { fields: [farmerCallbackRequestsTable.farmerId], references: [farmersTable.id] }),
}));
...
export const insertFarmerCallbackRequestSchema = createInsertSchema(farmerCallbackRequestsTable).omit({ id: true, createdAt: true });
export const insertPaymentReviewSchema = createInsertSchema(paymentReviewsTable).omit({ id: true, createdAt: true });
...
export type FarmerCallbackRequest = typeof farmerCallbackRequestsTable.$inferSelect;
export type InsertFarmerCallbackRequest = z.infer<typeof insertFarmerCallbackRequestSchema>;
export type PaymentReview = typeof paymentReviewsTable.$inferSelect;
