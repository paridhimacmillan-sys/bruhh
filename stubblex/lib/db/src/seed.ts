...
const users = [
  { phone: "9876500001", name: "Amandeep Singh", role: "admin" as const },
  { phone: "9876500002", name: "Mehar Kaur", role: "coordinator" as const },
  { phone: "9876500003", name: "Jagmeet Singh", role: "operator" as const },
  { phone: "9876500004", name: "Simran Kaur", role: "operator" as const },
  { phone: "9876500005", name: "Gursharan Singh", role: "aggregator" as const },
  { phone: "9876500006", name: "Navjot Kaur", role: "inspector" as const },
] as const;
...
const onboardingApplications = [
  ...
  { reference: "STX-DEMO-1002", applicantType: "machine_partner" as const, name: "Manjit Agro Services", phone: "9814112202", district: "Sangrur", phoneVerifiedAt: new Date("2025-10-01T08:40:00Z"), applicationData: { machineType: "baler", machineCount: 3, serviceRadiusKm: 55, availabilityWindow: "15 Oct – 30 Nov" } },
  { reference: "STX-DEMO-1003", applicantType: "logistics_operator" as const, name: "Gill Rural Logistics", phone: "9814112203", district: "Sangrur", phoneVerifiedAt: new Date("2025-10-01T08:50:00Z"), applicationData: { machineType: "truck", machineCount: 5, serviceRadiusKm: 80, availabilityWindow: "October – February" } },
...
export async function seedDemoData(): Promise<void> {
  await db.transaction(async (tx) => {
    ...
    await tx.insert(usersTable).values([...users]).onConflictDoNothing();
    ...
    for (const farmer of farmers) {
      ...
      await tx
        .insert(farmersTable)
        .values({ name: farmer.name, phone: farmer.phone, fpoName: farmer.fpoName, clusterId, assignedOperatorId, listedTonnes: 20 })
        .onConflictDoUpdate({
          target: [farmersTable.name, farmersTable.clusterId],
          set: { phone: farmer.phone, fpoName: farmer.fpoName, assignedOperatorId, listedTonnes: 20 },
        });
    }
