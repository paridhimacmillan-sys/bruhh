...
const bootstrapAdminEmail = (process.env.BOOTSTRAP_ADMIN_EMAIL ?? "admin@unpackos.in").trim().toLowerCase();
const users = [
...
export async function seedDemoData(): Promise<void> {
  await db.transaction(async (tx) => {
    ...
    await tx.insert(usersTable).values([...users]).onConflictDoUpdate({
      ...
      set: { email: sql`excluded.email`, name: sql`excluded.name`, role: sql`excluded.role`, active: true },
    });
