...
OTP and notification SMS use MSG91's Flow API when `MSG91_AUTH_KEY` and the matching template ID are configured. Set `MSG91_OTP_TEMPLATE_ID` for login and applicant-verification codes, `MSG91_FARMER_TEMPLATE_ID` for paid-batch notifications, `MSG91_ONBOARDING_TEMPLATE_ID` for general application decisions, `MSG91_FARMER_APPROVAL_TEMPLATE_ID` for farmer acceptance and assigned-operator details, and `MSG91_QUANTITY_UPDATE_TEMPLATE_ID` for approved quantity increases. When MSG91 configuration is absent, complete messages are written to the API console for local demos.
