...
export function getDashboardData(date: string, shift: string | 'all') {
  ...
  const trendHourCount = filteredEntries.reduce(
    (max, entry) => Math.max(max, entry.entries.length),
    0
  );
  ...
  const loggedHours = filteredEntries.reduce(
    (s, e) => s + e.entries.filter((h) => h.actual > 0).length,
    0
  );
