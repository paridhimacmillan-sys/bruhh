      ...
      <div className="w-full max-w-sm space-y-8">
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          <div className="text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">Production Monitor — Sign in to continue</p>
          </div>
        </div>

        {/* Login card */}
        <div className="card-base p-8 rounded-xl space-y-5">
          <div>
            <h2 className="text-base font-semibold text-foreground">Sign in</h2>
            <p className="text-xs text-muted-foreground mt-0.5">Enter the username and password given by your admin</p>
          </div>
          ...
          <form action={loginWithCredentials} className="space-y-3">
            ...
            <button
              type="submit"
              className="w-full rounded-md bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground hover:bg-primary/90 active:scale-[0.98] transition-all mt-1"
            >
              Sign In
            </button>
          </form>

          {/* Google OAuth — secondary, for admins only */}
          <div className="pt-2 border-t border-border text-center">
            <p className="text-xs text-muted-foreground mb-2">Admin with Google account?</p>
            <a
              href={`/api/login/google?callbackUrl=${encodeURIComponent(callbackUrl)}`}
              className="inline-flex items-center gap-2 text-xs font-medium text-muted-foreground hover:text-foreground transition-colors underline-offset-4 hover:underline"
            >
